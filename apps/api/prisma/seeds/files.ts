...
export async function seedImaging(
  ...
  const s3 = getS3Client();
  if (!s3) {
    console.log(`⏭️  S3 non configuré, imaging ignoré`);
    return null;
  }
  if (!imagingIds.length) return [];

  const absolutePath = path.resolve(__dirname, localImagePath);
  const buffer = await readFile(absolutePath);

  const files = await Promise.all(
    imagingIds.map(async (id) => {
      const key = `imagings/${id}/${crypto.randomUUID()}`;
...
export async function seedAnalyses(
  ...
  const s3 = getS3Client();
  if (!s3) {
    console.log(`⏭️  S3 non configuré, analyse ignorée`);
    return null;
  }
  ...
  const files = await Promise.all(
    analysisIds.map(async (id) => {
      const key = `analysiss/${id}/${crypto.randomUUID()}`;
