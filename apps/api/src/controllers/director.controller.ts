...
export class DirectorController {
  ...
  async createVeterinarian(req: Request, res: Response, next: NextFunction) {
    try {
      const user = await directorService.createVeterinarian(req.user!.id, req.body);
      res.status(201).json(user);
    } catch (err) {
      next(err);
    }
  ...
  async createSecretary(req: Request, res: Response, next: NextFunction) {
    try {
      const user = await directorService.createSecretary(req.user!.id, req.body);
      res.status(201).json(user);
    } catch (err) {
      next(err);
    }
  ...
  async requestClinic(req: Request, res: Response, next: NextFunction) {
    try {
      const request = await directorService.requestClinic(req.user!.id, req.body);
