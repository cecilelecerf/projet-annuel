import { ReferentService } from "@api/services/referent.service";
...
export class ReferentController {
  ...
  async createVeterinarian(req: Request, res: Response, next: NextFunction) {
    try {
      const user = await referentService.createVeterinarian(req.user!.id, req.body);
  ...
  async createSecretary(req: Request, res: Response, next: NextFunction) {
    try {
      const user = await referentService.createSecretary(req.user!.id, req.body);
      res.status(201).json(user);
    } catch (err) {
      next(err);
    }
