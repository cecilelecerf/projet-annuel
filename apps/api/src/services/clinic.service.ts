...
import { CreateClinic, UpdateClinic } from "@armali/schemas";
...
export class ClinicService {
  ...
  async getClinicStaff(userId: string, role: string) {
    ...
    if (role === "VETERINARIAN") {
      const vc = await prisma.veterinarianClinic.findFirst({ where: { veterinarianId: userId } });
      clinicId = vc?.clinicId ?? null;
    } else if (role === "SECRETARY") {
      const sp = await prisma.secretaryProfile.findUnique({ where: { id: userId } });
      clinicId = sp?.clinicId ?? null;
    } else if (role === "DIRECTOR") {
      const dp = await prisma.directorClinicProfile.findUnique({ where: { id: userId } });
      clinicId = dp?.clinicId ?? null;
    } else if (role === "REFERANT") {
      const rp = await prisma.referentClinicProfile.findUnique({ where: { id: userId } });
  ...
  async updateClinic(userId: string, data: UpdateClinic) {
    ...
    if (!profile) throw new BadRequestError("Aucune clinique associée à ce compte directeur");
