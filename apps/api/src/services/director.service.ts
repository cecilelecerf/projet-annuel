...
import { BadRequestError, ConflictError, ForbiddenError } from "@api/errors";
import type {
  CreateReferentStaff,
...
export class DirectorService {
  ...
  async createVeterinarian(directorUserId: string, data: CreateVeterinarianStaff) {
    const clinicId = await this.getClinicId(directorUserId);
    const hashedPassword = await hash(data.password, 10);
  ...
  async getClinicStatus(directorUserId: string) {
    ...
    if (latestRequest.status === "PENDING") return { status: "PENDING" as const, request: latestRequest };
    if (latestRequest.status === "REJECTED") return { status: "REJECTED" as const, request: latestRequest };
  ...
  async requestClinic(directorUserId: string, data: CreateClinicRequest) {
    ...
    if (profile) throw new BadRequestError("Vous avez déjà une clinique approuvée");

    const pendingRequest = await prisma.clinicCreationRequest.findFirst({
      where: { directorId: directorUserId, status: "PENDING" },
    });
    if (pendingRequest) throw new ConflictError("Vous avez déjà une demande en attente de validation");

    const existingClinic = await prisma.clinic.findUnique({ where: { siret: data.siret } });
    ...
    if (siretPending) throw new ConflictError("Une demande avec ce numéro SIRET est déjà en attente");
