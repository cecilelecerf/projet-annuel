...
describe("SpecialityService.getAllByClinicId", () => {
  it("retourne les spécialités de la clinique", async () => {
    const specialities = [makeSpeciality()];
    mockRepository.findAllByClinicId.mockResolvedValue({
      id: CLINIC_ID,
      specialities,
    });

    const result = await service.getAllByClinicId({
      clinicId: CLINIC_ID as any,
    });

    expect(mockRepository.findAllByClinicId).toHaveBeenCalledWith({
      clinicId: CLINIC_ID,
    });
    expect(result).toEqual(specialities);
  });

  it("clinique introuvable — retourne un tableau vide", async () => {
    mockRepository.findAllByClinicId.mockResolvedValue(null);
...
describe("SpecialityService.linkWithClinic", () => {
  it("met à jour les spécialités de la clinique et retourne la liste", async () => {
    const specialities = [makeSpeciality()];
    mockRepository.linkWithClinic.mockResolvedValue({
      id: CLINIC_ID,
      specialities,
    });

    const data = { specialityIds: [SPECIALITY_ID] };
    const result = await service.linkWithClinic({
    ...
    expect(mockRepository.linkWithClinic).toHaveBeenCalledWith({
      clinicId: CLINIC_ID,
      data,
    });
    expect(result).toEqual(specialities);
