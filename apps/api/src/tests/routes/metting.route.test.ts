...
const mockMettingService = vi.hoisted(() => ({
  getMettingsForVeterinarian: vi.fn(),
  getMettingsForSecretary: vi.fn(),
  getMettingsForReferant: vi.fn(),
...
vi.mock("@api/services/metting.service", () => ({
  MettingService: vi.fn(function () {
    return mockMettingService;
  }),
}));
...
describe("GET /api/meetings", () => {
  ...
  describe("VETERINARIAN", () => {
    ...
    it("200 — retourne le calendrier du vétérinaire", async () => {
      mockMettingService.getMettingsForVeterinarian.mockResolvedValue([
        mockFlatMeeting,
      ]);
      mockMettingService.getAllAvailibilities.mockResolvedValue([
        mockAvailability,
      ]);
      ...
      expect(
        mockMettingService.getMettingsForVeterinarian,
      ).toHaveBeenCalledWith("veto-1", expect.any(Date), expect.any(Date));
    });

    it("404 — profil vétérinaire introuvable", async () => {
      mockMettingService.getMettingsForVeterinarian.mockResolvedValue(null);
      mockMettingService.getAllAvailibilities.mockResolvedValue([]);
  ...
  describe("SECRETARY", () => {
    ...
    it("200 — retourne le calendrier de la secrétaire", async () => {
      mockMettingService.getMettingsForSecretary.mockResolvedValue([
        mockFlatMeeting,
      ]);
      mockMettingService.getAllAvailibilities.mockResolvedValue([
        mockAvailability,
      ...
      expect(res.status).toBe(200);
      expect(mockMettingService.getMettingsForSecretary).toHaveBeenCalledWith(
        "sec-1",
        expect.any(Date),
        expect.any(Date),
      );
    ...
    it("404 — profil secrétaire introuvable", async () => {
      mockMettingService.getMettingsForSecretary.mockResolvedValue(null);
      mockMettingService.getAllAvailibilities.mockResolvedValue([]);
...
describe("GET /api/meetings/:veterinarianId", () => {
  ...
  describe("SECRETARY", () => {
    ...
    it("200 — retourne le calendrier d'un vétérinaire", async () => {
      ...
      mockMettingService.getAvailibilitiesByClinic.mockResolvedValue([
        mockAvailability,
      ]);
