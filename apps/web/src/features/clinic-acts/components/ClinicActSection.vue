...
import { useClinicActs } from '../composables/useClinicActs'
import { useClinicActForm } from '../composables/useClinicActForm'
import { useClinicActFilters } from '../composables/useClinicActFilters'
...
const editingActId = computed(() => {
  ...
  return clinicActs.value.find((ca) => ca.id === editingId.value)?.actId ?? null
})
...
async function submitForm() {
  ...
  try {
    if (mode.value === 'create') {
      await createClinicAct(props.clinicId, form.value)
    } else if (editingId.value) {
      await updateClinicAct(props.clinicId, editingId.value, form.value)
    }
    close()
  } finally {
...
async function handleDelete(clinicAct: ClinicAct) {
  actionLoading.value = true
  try {
    await deleteClinicAct(props.clinicId, clinicAct.id)
  } finally {
    actionLoading.value = false
  }
}
