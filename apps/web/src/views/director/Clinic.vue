...
onMounted(async () => {
  try {
    const data = await api('/director/clinic')
...
async function saveClinic() {
  saving.value = true
  try {
    clinic.value = await api('/clinics/me', {
      method: 'PATCH',
      body: JSON.stringify(editClinic.value),
    })
...
async function submitRequest() {
  ...
  if (form.siret.replace(/\s/g, '').length !== 14) return notify.error('Le SIRET doit contenir 14 chiffres')
  ...
  try {
    await api('/director/clinics/request', {
      method: 'POST',
      body: JSON.stringify({
        ...form,
...
<template>
  <div class="clinic-page">
    ...
    <div v-else-if="status === 'PENDING'" class="card">
      ...
      <p class="state-desc">
        Votre demande a été transmise à un administrateur. Vous serez notifié dès qu'elle sera traitée.
        Une fois approuvée, vous pourrez gérer votre clinique et créer des comptes pour votre personnel.
      </p>
      <div v-if="request" class="request-summary">
        <div class="info-row"><span class="info-label">Clinique</span><span class="info-value">{{ request.name }}</span></div>
        <div class="info-row"><span class="info-label">Adresse</span><span class="info-value">{{ request.address }}</span></div>
        <div class="info-row"><span class="info-label">SIRET</span><span class="info-value">{{ request.siret }}</span></div>
        <div class="info-row">
          <span class="info-label">Demande envoyée le</span>
          <span class="info-value">{{ new Date(request.createdAt).toLocaleDateString('fr-FR') }}</span>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="status === 'REJECTED'">
      <div class="card rejection-banner">
        ...
        <p class="state-desc">
          Votre précédente demande a été refusée. Vous pouvez soumettre une nouvelle demande ci-dessous.
        </p>
        <div v-if="request" class="request-summary">
          <div class="info-row"><span class="info-label">Clinique refusée</span><span class="info-value">{{ request.name }}</span></div>
          <div class="info-row"><span class="info-label">SIRET</span><span class="info-value">{{ request.siret }}</span></div>
        </div>
      </div>
    ...
    <div v-else-if="status === 'NONE'" class="card">
      ...
      <p class="state-desc">Aucune demande en cours. Contactez un administrateur si vous pensez qu'il s'agit d'une erreur.</p>
    </div>
    ...
    <div v-else-if="status === 'APPROVED' && clinic" class="card">
      ...
      <template v-if="!editMode">
        <div class="info-grid">
          <div class="info-row"><span class="info-label">Nom</span><span class="info-value">{{ clinic.name }}</span></div>
          <div class="info-row"><span class="info-label">Adresse</span><span class="info-value">{{ clinic.address }}</span></div>
          <div class="info-row"><span class="info-label">SIRET</span><span class="info-value">{{ clinic.siret }}</span></div>
          <div class="info-row"><span class="info-label">Téléphone</span><span class="info-value">{{ clinic.phone }}</span></div>
          <div class="info-row">
            <span class="info-label">Site web</span>
            <span class="info-value"><a :href="clinic.website" target="_blank">{{ clinic.website }}</a></span>
          </div>
          <div v-if="clinic.description" class="info-row">
            <span class="info-label">Description</span><span class="info-value">{{ clinic.description }}</span>
          </div>
...
<style scoped>
.clinic-page { padding: 32px; max-width: 720px; margin: auto; }
.card {
  background: white;
  border-radius: 12px;
  padding: 32px;
...
.rejection-banner { border-left: 4px solid #f56c6c; }
.state-icon { font-size: 48px; text-align: center; margin-bottom: 12px; }
.state-title { font-size: 20px; font-weight: 700; text-align: center; margin: 0 0 8px; }
.state-desc { color: #666; text-align: center; margin: 0 0 24px; line-height: 1.6; }
...
.request-summary { background: #f8f9fa; border-radius: 8px; padding: 16px; display: flex; flex-direction: column; gap: 10px; }
.info-row { display: flex; justify-content: space-between; align-items: flex-start; }
.info-label { color: #888; font-size: 14px; flex-shrink: 0; margin-right: 16px; }
