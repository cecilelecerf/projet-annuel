...
async function loadStaff() {
  try {
    staff.value = await api('/director/staff')
  } catch {
...
async function submitReferent() {
  ...
  try {
    await api('/director/staff/referents', { method: 'POST', body: JSON.stringify(referentForm) })
...
async function submitVet() {
  ...
  try {
    await api('/director/staff/veterinarians', { method: 'POST', body: JSON.stringify(vetForm) })
...
async function submitSecretary() {
  ...
  try {
    await api('/director/staff/secretaries', { method: 'POST', body: JSON.stringify(secretaryForm) })
  ...
  <div class="staff-page">
    ...
    <div v-else-if="clinicStatus !== 'APPROVED'" class="gate-card">
      ...
      <p v-if="clinicStatus === 'PENDING'" class="gate-desc">
        Votre demande de clinique est <strong>en attente de validation</strong>. Vous pourrez créer des comptes
        dès qu'un administrateur l'aura approuvée.
      </p>
      <p v-else class="gate-desc">
        Vous devez avoir une clinique approuvée avant de pouvoir créer des comptes pour votre personnel.
      </p>
        ...
        <div class="form-card">
          ...
          <el-form label-position="top" @submit.prevent="submitReferent">
            ...
            <el-form-item label="Email">
              <el-input v-model="referentForm.email" type="email" placeholder="email@exemple.com" />
            </el-form-item>
            <el-form-item label="Mot de passe provisoire">
              <el-input v-model="referentForm.password" type="password" show-password placeholder="Minimum 8 caractères" />
            </el-form-item>
...
.gate-title { font-size: 20px; font-weight: 700; margin: 0 0 12px; color: #1a1a1a; }
.gate-desc { color: #6b7280; margin: 0 0 24px; line-height: 1.6; max-width: 420px; margin-left: auto; margin-right: auto; }
.staff-list-card {
  background: white;
  border-radius: 12px;
  box-shadow: 0 1px 4px rgba(0,0,0,0.08);
