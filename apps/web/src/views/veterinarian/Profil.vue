...
interface ClinicData { name: string; address: string; phone: string; website: string; description?: string | null; openingHours?: string | null }
interface StaffMember { id: string; firstname: string; lastname: string; email: string; role: string }
interface StaffList { director: StaffMember | null; referents: StaffMember[]; veterinarians: StaffMember[]; secretaries: StaffMember[] }

const clinic = ref<ClinicData | null>(null)
const staff = ref<StaffList | null>(null)

const roleLabel: Record<string, string> = { DIRECTOR: 'Directeur', REFERANT: 'Référent', VETERINARIAN: 'Vétérinaire', SECRETARY: 'Secrétaire' }
const roleTag: Record<string, string> = { DIRECTOR: 'danger', REFERANT: '', VETERINARIAN: 'success', SECRETARY: 'warning' }
  ...
  <div class="profil-page">
    ...
    <div v-if="clinic" class="profil-card" style="margin-top: 24px">
      ...
      <div class="profil-info">
        ...
        <div v-if="clinic.description" class="info-row"><span class="info-label">Description</span><span class="info-value">{{ clinic.description }}</span></div>
...
<style scoped>
.profil-page { padding: 32px; max-width: 600px; margin: auto; }
...
.profil-avatar { width: 72px; height: 72px; border-radius: 50%; color: white; display: flex; align-items: center; justify-content: center; font-size: 24px; font-weight: 700; flex-shrink: 0; background: #67c23a; }
.profil-name { font-size: 22px; font-weight: 700; margin: 0 0 4px; }
.profil-role { background: #f0f9eb; color: #67c23a; border-radius: 6px; padding: 2px 10px; font-size: 13px; font-weight: 500; }
...
.staff-item { display: flex; align-items: center; gap: 12px; padding: 8px 0; border-bottom: 1px solid #f3f4f6; }
.staff-item:last-child { border-bottom: none; }
.staff-avatar { width: 36px; height: 36px; border-radius: 50%; background: #e0e7ff; color: #4f46e5; display: flex; align-items: center; justify-content: center; font-size: 13px; font-weight: 700; flex-shrink: 0; }
.staff-info { flex: 1; }
