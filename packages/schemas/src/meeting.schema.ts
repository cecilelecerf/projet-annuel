import { z } from 'zod'
import { meetingIdSchema, ownedPetIdSchema, veterinarianClinicIdSchema, specialityIdSchema } from './ids'

// ── Meeting ───────────────────────────────────────────────────────────────────
export const meetingStatusSchema = z.enum(['pending', 'confirmed', 'done', 'cancelled'])
...
export const meetingSchema = z.object({
  ...
  { message: 'endDatetime doit être après startDatetime', path: ['endDatetime'] }
)

export const createMeetingSchema = meetingSchema.omit({ id: true, createdAt: true, modifiedAt: true })
